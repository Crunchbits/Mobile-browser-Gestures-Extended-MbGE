// ==UserScript==
// @name mobile browser touch gestures (English translated)
// @name:en Mobile browser touch gestures (English translated)
...
// ***********************************************************************
// English translations made by Crunchbits https://github.com/Crunchbits
// ***********************************************************************
/*Gesture Data Module*/
let gesture = {
    '↑→↓←': 'Open settings',
    ...
    '→↓↑←': 'Video analysis',
    'T→↑': 'Baidu translation',
    'T←↑': 'Youdao translation',
    'T◆◆': 'Query search',
    'I↓↑●': 'Open the picture',
    'I→↑●': 'Baidu search picture',
    'V→': 'Forward 10s',
    'V←': 'Back 10s',
    'V↑': 'Increase speed',
    'V↓': 'Decrease speed',
    ...
    pathFn = {
        ...
        'Back to bottom': '/*WITH TOP*/let boxNode=gestureData.touchEle.parentNode;while(boxNode.nodeName!=="#document"){if(getComputedStyle(boxNode).overflowY!=="hidden"){boxNode.scrollTo(0,boxNode.scrollHeight+999999);}boxNode=boxNode.parentNode;}',
        'Refresh page': '/*ONLY TOP*/document.documentElement.style.cssText="filter:grayscale(100%)";history.go(0);',
        'New page': '/*ONLY TOP*/GM_openInTab("//limestart.cn");',
        ...
        'Video analysis': '/*ONLY TOP*/GM_openInTab("//jx.bozrc.com:4433/player/?url="+location.href);',
        'Baidu translation': 'GM_openInTab("//fanyi.baidu.com/#auto/auto/"+encodeURIComponent(gestureData.selectWords));',
        'Youdao translation': 'GM_openInTab("//dict.youdao.com/w/eng/"+encodeURIComponent(gestureData.selectWords));',
        ...
        'Baidu search picture': 'GM_openInTab("//graph.baidu.com/details?isfromtusoupc=1&tn=pc&carousel=0&promotion_name=pc_image_shituindex&extUiData%5bisLogoShow%5d=1&image="+gestureData.touchEle.src);',
        'Forward 10s': 'videoPlayer.currentTime+=10;gestureData.tipBox.innerHTML="+10s ";gestureData.tipBox.style.display="block";setTimeout(()=>{gestureData.tipBox.style.display="none";},500);',
        'Back 10s': 'videoPlayer.currentTime-=10;gestureData.tipBox.innerHTML="-10s ";gestureData.tipBox.style.display="block";setTimeout(()=>{gestureData.tipBox.style.display="none";},500);',
        'Increase speed': 'if(document.fullscreen){let playSpeed=videoPlayer.playbackRate;playSpeed+=(playSpeed<1.5) ? 0.25 : 0.5;gestureData.tipBox.innerHTML="×"+playSpeed+" ∞ ";gestureData.tipBox.style.display="block";videoPlayer.playbackRate=playSpeed;setTimeout(()=>{gestureData.tipBox.style.display="none";},500)}',
        'Decrease speed': 'if(document.fullscreen){let playSpeed=videoPlayer.playbackRate;playSpeed-=(playSpeed>1.5) ? 0.5 : (playSpeed>0.25 && 0.25);gestureData.tipBox.innerHTML="×"+playSpeed+" ∞ ";gestureData.tipBox.style.display="block";videoPlayer.playbackRate=playSpeed;setTimeout(()=>{gestureData.tipBox.style.display="none";},500)}',
        ...
        'Release fast forwarding': 'videoPlayer.playbackRate=gestureData.playSpeed;gestureData.tipBox.style.display="none";',
        ...
        'Release swipe right progress': 'clearInterval(gestureData.videoTimer);gestureData.tipBox.style.display="none";',
        'Swipe left progress': 'let lastX=gestureData.touchEnd.screenX,rem=videoPlayer.currentTime/60,mod=videoPlayer.currentTime%60;gestureData.tipBox.innerHTML=((rem<10) ? "0" : "")+(rem|0)+" : "+((mod<10) ? "0" : "")+(mod|0);gestureData.tipBox.style.display="block";gestureData.videoTimer=setInterval(()=>{if(gestureData.touchEnd.screenX-lastX){videoPlayer.currentTime+=(gestureData.touchEnd.screenX-lastX)*(1+Math.abs(gestureData.touchEnd.screenX-lastX)/20);lastX=gestureData.touchEnd.screenX;}rem=videoPlayer.currentTime/60;mod=videoPlayer.currentTime%60;gestureData.tipBox.innerHTML=((rem<10) ? "0" : "")+(rem|0)+" : "+((mod<10) ? "0" : "")+(mod|0);},50);',
        'Release swipe left progress': 'clearInterval(gestureData.videoTimer);gestureData.tipBox.style.display="none";'
    },
    settings = {
...
function touchStart(e) {
    ...
    if (timeSpan > 50 || lineLen > limit) {//Disconnection judgment
        ...
        if (!gestureTimer || lineLen > limit) {
            ...
            if (gestureData.selectWords && settings['Text gestures']) { path = 'T'; }
            else if (document.contains(videoPlayer) && settings['Video gestures']) {
...
function openSet() {
    ...
    addStyle('*{overflow:hidden !important;}' +
        ...
        '#gestureBox #revisePath div{color:#3339f9;position:absolute;width:30%;height:12vmin;font-size:9vmin;bottom:15%;}' +
    ...
    gestureBox.innerHTML = '<h1 id="openSettings">Gesture track settings</h1><div id="addGesture">+</div><div id="gestureUL"></div>' +
        ...
        '<div id="settingsBox"><h1>Function switch settings</h1><span id="settingList"></span><button id="saveSettings">save</button></div>';
    ...
    function delGesture() {
        ...
        if (!confirm('Confirm to delete "' + gestureName + '" gesture:')) { return; }
        gesturePath = this.parentNode.getAttribute('path');
